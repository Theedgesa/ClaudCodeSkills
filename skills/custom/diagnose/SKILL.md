...
**Then ask: what would have prevented this bug?** If the answer involves architectural change, note it for future work.

## Phase 6.5 — Register in Roadmap

If a new bug was identified during this diagnosis session:

1. Allocate PROJ-NNN from `.claude/work/.next-ppv3` (read current value, increment, write back)
2. Create entry in `.claude/work/roadmap.yaml`:
   - `stage:` depends on fix status (`closed` if fixed inline, `idea` if unfixed)
   - `discovered_during:` the PROJ-NNN of the work being done when bug was found
   - `theme:` based on what the bug affects (platform/tenant/customer)
   - `priority:` based on severity
3. Run the **Monitoring Scan** (see below)

If new work was identified (not a bug — a feature gap, improvement, etc.):

1. Same allocation + creation, `stage: idea`
2. Run the monitoring scan
