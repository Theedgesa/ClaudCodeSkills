...
Invoke /impeccable-impeccable, which contains design principles, anti-patterns, and the **Context Gathering Protocol**. Follow the protocol before proceeding. If no design context exists yet, you MUST run /impeccable-impeccable teach first.
...
**Output**: A design brief that can be handed off to /impeccable-impeccable craft, /impeccable-impeccable, or any other implementation skill.
...
Once confirmed, the brief is complete. The user can now hand it to /impeccable-impeccable, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use /impeccable-impeccable craft instead, which runs this skill internally.)